...
class trainModel(TemplateView):
    ...
    def put(self,request):
        try:
            ...
            print(f"len train : {len(train)}\t len validate : {len(validate)}") #debuging
...
# obtienes la ruta absuoluta de este archivo en el sistema operativo
            with open(os.path.dirname(os.path.abspath(__file__))+"/tempModels/CountVect.pickle",'wb') as file:  #pickle te ayuda a guardar ojetos de python en binario
                pickle.dump(count_vect,file)
            with open(os.path.dirname(os.path.abspath(__file__))+"/tempModels/tfidfTransformer.pickle",'wb') as file: #que se guarde en tempModelsy que haga una escritura binaria 
